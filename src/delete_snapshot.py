...
class DeleteSnapshotManager(BaseManager):
    def __init__(self, config_file: str, del_day: str, **arg):
        super().__init__()
        ...
        self.del_date = self.calculate_del_date(del_day)

    @staticmethod
    def calculate_del_date(del_day: str) -> str:
        """
        del_day 인자 체크

        :param del_day: 오늘부터 며칠 전의 스냅샷을 삭제할지 지정

        :return del_date: 삭제 진행할 날짜
        """
        try:
            if del_day[-1] == "d":
                del_day = int(del_day[:-1])
                return (datetime.now() - timedelta(days=del_day)).strftime("%Y-%m-%d")
            else:
                raise ValueError

        except ValueError as e:
            _LOGGER.error(f"del_day 변수 형식이 잘못되었습니다. 13d와 같이 숫자+d로 작성해주세요: {del_day}")
...
if __name__ == "__main__":
    ...
    delete_snapshot_manager = DeleteSnapshotManager(arg_dict["config"], arg_dict["del_day"])
    delete_snapshot_manager.delete_snapshot()
